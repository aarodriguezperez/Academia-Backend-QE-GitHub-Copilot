...
Al terminar las evidencias y el último push debo revisar y eliminar o detener los recursos que ya no sean necesarios:

- [ ] `taskflow-pipeline`.
- [ ] Proyecto CodeBuild `taskflow-build`.
- [ ] Aplicación y deployment group de CodeDeploy.
- [ ] Instancia EC2 `taskflow-ec2`.
- [ ] RDS `taskflow-db`.
- [ ] Bucket y artefactos de S3.
- [ ] Roles IAM creados específicamente para la práctica.
- [ ] Security Groups temporales.
- [ ] Access Key creada para `taskflow-admin`.
- [ ] Revisión final de Billing/Budgets.
